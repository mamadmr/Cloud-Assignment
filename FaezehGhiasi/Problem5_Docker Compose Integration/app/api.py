...
def assign_challenge(request: ChallengeRequest,db: Session = Depends(get_db),):
    ...
    dup = (db.query(AssignedChallenge).filter_by(team_id=request.team_id, challenge_id=request.challenge_id).first())
    if dup.status:
        raise HTTPException(
    ...
    start_container.delay(image_name, container_name)

    assignment = AssignedChallenge(
        team_id=request.team_id,
        challenge_id=request.challenge_id,
        container_name=container_name,
        container_address="http://localhost",  
        status="starting",
    )
    db.add(assignment)
    db.commit()
